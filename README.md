...
| `--prompt-file <FILE>` | Read a prompt from a file to prepend to the generated context. | 
...
| `--no-git-root` | Disable using the root of the git project (overrides config and `--git-root`). | 
| `--tree` | Show *only* the directory tree structure (no file contents). | 
| `--include <GLOBS>` | Patterns for files to include content (e.g. `**/*.rs`). | 
| `--include-in-tree <GLOBS>` | Patterns for files to show in the structural tree, but skip contents. | 
...
| `--sql` | Include database schema context (reads `DB_URL` from env). | 
| `--db-url <URL>` | Optional database connection string (triggers `--sql` automatically). | 
| `--samples` | Collect sample rows from tables (SQL mode only). | 
| `--max-sample-len <LEN>` | Max length for string values in database sample rows (default: `256`). | 
